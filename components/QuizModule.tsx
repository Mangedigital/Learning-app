    ...
    <div className="space-y-5 md:space-y-8">
      <div className="flex justify-between items-center px-1">
        ...
        <div className="flex gap-1 md:gap-1.5">
          {filteredQuestions.map((_, i) => (
            <div key={i} className={`h-1.5 md:h-2 w-5 md:w-8 rounded-full transition-all duration-300 ${i <= currentIdx ? 'bg-[#004b89]' : 'bg-slate-100'}`}></div>
          ))}
        </div>
      </div>

      <div className="text-lg md:text-2xl font-semibold text-slate-800 text-center py-4 md:py-6 px-2 min-h-[80px] md:min-h-[120px] flex items-center justify-center">
        {question.question}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-4">
        <button
          ...
          className={`py-5 md:py-8 rounded-2xl border-2 font-bold text-lg md:text-xl transition-all shadow-sm ${
            showExplanation
      ...
      {showExplanation && (
        <div className="animate-in fade-in slide-in-from-top-4 duration-300">
          ...
          <button
            onClick={next}
            className="w-full mt-4 md:mt-6 bg-[#004b89] text-white py-3 md:py-4 rounded-xl font-bold hover:bg-[#003d70] transition-all shadow-lg hover:-translate-y-0.5"
          >
